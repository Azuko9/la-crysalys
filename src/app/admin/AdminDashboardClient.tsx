...
export default function AdminDashboardClient({ initialMessages, userEmail }: AdminDashboardClientProps) {
  const [messages, setMessagesRecus] = useState<ContactMessage[]>(initialMessages);
  ...
  const handleDeleteMessage = useCallback(async (id: string) => {
    ...
    if (result.success) {
      ...
      toast.error('error' in result && result.error ? String(result.error) : "Erreur lors de la suppression du message.");
    }
  }, []);
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center justify-between mb-10">
              <h2 className="text-2xl font-black italic uppercase flex items-center gap-4">
                <div className="w-10 h-10 bg-primary flex items-center justify-center text-black"><Mail size={20} /></div>
                  ...
                  <div key={msg.id} className="bg-card border border-zinc-800 p-8 rounded-dynamic relative group hover:border-primary transition-all duration-500">
                    ...
                    <div className="mb-8 pr-12">
                        <div className="flex items-center gap-4 mb-4">
                           <span className="bg-card text-foreground/70 text-[9px] font-black px-3 py-1 uppercase tracking-widest border border-zinc-700">Reception_OK</span>
                    ...
                    <div className="flex items-center gap-4">
                      <a href={`mailto:${msg.email}`} className="inline-flex items-center text-[10px] font-black uppercase tracking-[0.3em] text-primary hover:text-foreground transition-all gap-4 group">
                        <div className="w-12 h-12 bg-card border border-zinc-800 rounded-dynamic flex items-center justify-center group-hover:bg-primary group-hover:text-black transition-all"><ExternalLink size={16} /></div>
                        Reply to : {msg.email}
                      </a>
                      <button onClick={() => handleCopyEmail(msg.email)} className="inline-flex items-center text-[10px] font-black uppercase tracking-[0.3em] text-foreground/50 hover:text-primary transition-all gap-4 group" title="Copier l'adresse email">
                        <div className="w-12 h-12 bg-card border border-zinc-800 rounded-dynamic flex items-center justify-center group-hover:border-primary transition-all"><Copy size={16} /></div>
                      </button>
                    </div>
                  </div>
                ))}

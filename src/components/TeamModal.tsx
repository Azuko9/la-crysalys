      ...
      <div ref={modalRef} className="bg-card border border-zinc-800 p-8 rounded-[2.5rem] w-full max-w-2xl shadow-2xl my-8 text-foreground relative">
        ...
        <div className="flex justify-between items-center mb-8 border-b border-zinc-800 pb-6">
          <h3 className="text-3xl font-black italic uppercase tracking-tighter text-primary">
            {member ? 'Modifier' : 'Ajouter'} <span className="text-foreground">{isPartner ? 'Partenaire' : 'Talent'}</span>
          </h3>
          <button onClick={onClose} className="text-foreground/40 hover:text-foreground transition-colors"><XCircle size={32}/></button>
        </div>
             ...
             {currentPhotoPublicUrl ? (
                <div className="relative w-full h-64 rounded-dynamic overflow-hidden border border-zinc-700 group">
                   ...
                   <div className="absolute inset-0 bg-black/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                      <button type="button" onClick={handleRemoveImage} className="bg-red-600 hover:bg-red-500 text-foreground px-4 py-2 rounded-full font-bold text-xs uppercase tracking-widest flex items-center gap-2"><Trash2 size={16}/> Changer</button>
                   </div>
                </div>
             ) : (
